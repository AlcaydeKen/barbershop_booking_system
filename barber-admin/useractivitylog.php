...
if(isset($_SESSION['username_barbershop_Xw211qAAsq4']) && isset($_SESSION['password_barbershop_Xw211qAAsq4']))
{
    ...
    logAdminActivity($_SESSION['admin_id_barbershop_Xw211qAAsq4'], 'Opened User Activity Log Dashboard');
    ...
    if ($admin) {
        ...
        if (isset($_POST['clearLogs'])) {
            ...
            $clearQuery = "DELETE FROM user_logs";
            $con->query($clearQuery);
            
            // Log the activity for clearing logs
            logAdminActivity($adminId, 'Cleared User Activity Logs');
        }
            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="adminLogsTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        $query = "SELECT user_logs.date, users.name AS username, user_logs.action 
                                  FROM user_logs
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            echo "<tr>";
                            echo "<td>{$row['date']}</td>";
                            echo "<td>{$row['username']}</td>"; // Display the admin's username
